...
if ($settings['type'] !== 'loadmore' || ($settings['type'] == 'loadmore' && $settings['class'] !== 'wpp_bottom_pagination')) {
  ...
  if ($hide_count != 'true') {
    ...
    $wpp_property_results .= ($properties['total'] > 0 ? \WPP_F::format_numeric($properties['total']) : __('No', ud_get_wp_property()->domain));
    $wpp_property_results .= __(' properties found. ', ud_get_wp_property()->domain);
  ...
  if ($sortable_attrs) {
    ...
    if ($settings['sorter_type'] == 'buttons') {
      foreach ($sortable_attrs as $slug => $label) {
        ...
        $sort_html .= '<div><span class="wpp_sortable_link ';
        if ($sort_by == $slug) {
          $sort_html .= 'wpp_sorted_element';
        }
//            $sort_html .= ($sort_by == $slug ? 'wpp_sorted_element' : '');
        $sort_html .= 'label label-info low-to-high" sort_order = "ASC" sort_slug = "' . $slug . '" >Price: Low to High</span > ';
        ...
        $sort_html .= 'label label-info high-to-low" sort_order = "DESC" sort_slug = "' . $slug . '" >Price: High to Low</span ></div>';
      }
